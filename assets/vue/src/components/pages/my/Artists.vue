...
let close_handle = watchEffect(async () => {
  ...
  get(`my-artists/?id=${id}`).then(res => data.value = res)
})
...
<template>
  ...
  <el-scrollbar v-if="data.concerns">
    <div v-for="(v) in data.concerns" :class="[c_row, '*:m-2 mx-8',
      ...
      <div :class="[c_col, 'basis-auto']">
        <div class="basis-1/3">
          <div class="text-[20px] stroke-2 hover:cursor-pointer hover:text-sky-500"
                   @click="router.push(`/g/Artist?id=${v.artist.id}`)"
          >{{v.nickname}}</div>
        </div>
        <div :class="c_row">
          <el-text line-clamp="2" class="hover:cursor-pointer hover:text-sky-500"
                   @click="router.push(`/g/Artist?id=${v.artist.id}`)"
          >{{v.artist.description}}</el-text>
        </div>
      </div>
    </div>
  </el-scrollbar>

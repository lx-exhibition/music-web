...
let close_handle = watchEffect(async () => {
  ...
  get(`e-Artists/`, {tag: tag, subtag: subtag}).then(res => data.value = res)
})
  ...
  <el-card shadow="hover">
    <template #header>
      <el-row align="middle">
        ...
        <el-col :span="1">
          <el-link :underline="false" @click="router.push('/explore/Artists')"><h1>入住歌手</h1></el-link>
        </el-col>
        <el-col :span="22">
          <el-tag v-for="(v) in data.tags" class="mx-2 hover:cursor-pointer" @click="router.push(`/explore/Artists?tag=${v.subtag}`)"
          >{{v.subtag}}</el-tag>
        </el-col>
      </el-row>
    </template>
    <template #default>
      <div class="grid grid-cols-5">
        <el-card v-for="(v) in data.artists" :class="[c_col,'w-36 h-40 drop-shadow-2xl wrap mt-4', c_hover]" shadow="hover"
                 @click="router.push(`/g/Artist?id=${v.id}`)"
        >

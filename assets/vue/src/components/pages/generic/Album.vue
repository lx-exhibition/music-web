...
watchEffect(async () => {
  ...
  get(`g-Album/?id=${id}`).then(res => data.value = res)
  get(`g-Comment/?type=album&id=${id}`).then(res => data_comment.value = res)
})
...
<template>
  <div :class="[c_row]">
    <div :class="['size-48 flex rounded-2xl', c_bg]">
      <el-image :src="data.background" :class="['size-36 rounded-full hover:animate-slow-spin', c_center]"/>
    </div>
    <div :class="[c_col, 'ml-5 *:mt-2 basis-3/6']">
      <p class="text-3xl">{{data.title}}</p>
      <p>
        <span class="text-[12px]">歌手:</span>
        <span :class="[c_hover]" v-if="data.artist"
              @click="router.push(`/g/Artist?id=${data.artist.id}`)">{{data.artist.stagename}}</span>
      </p>
      <p><span class="text-[12px]">发行时间:</span> {{data.pub_date}}</p>
      <p><span class="text-[12px]">发行公司:</span> {{data.pub_company}}</p>
      <div class="*:ml-2">
    ...
    <div :class="[c_col, 'basis-auto']">
      ...
      <div :class="c_row">
        <el-text line-clamp="1" :class="[c_hover]"
                 @click="router.push(`/g/Album?id=${data.id}`)"
        >{{data.title}}</el-text>
      </div>
  ...
  <div :class="[c_row, 'h-24 *:ml-2']">
    ...
    <el-button class="basis-12 max-h-sm my-auto" type="primary"
               @click="post('AlbumComment/', {
                 // user: uid,
                 user: 32+Math.ceil(Math.random()*20),
                 album: route.query.id, content: comment,
                 shared_count: Math.ceil(Math.random()*10000), liked_count: Math.ceil(Math.random()*10000)}
               ).then(()=>{
                 comment = ''
                 get(`g-Comment/?type=album&id=${route.query.id}`).then(res => data_comment = res)
               })"
    >评论</el-button>
  </div>
